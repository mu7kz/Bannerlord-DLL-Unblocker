...
function Find-ModulesFolder {
    ...
    # Check common default paths first
    ...
    # Last resort: scan fixed drives for common Steam folder names
    $drives = [System.IO.DriveInfo]::GetDrives() |
        Where-Object { $_.DriveType -eq 'Fixed' -and $_.IsReady } |
        Select-Object -ExpandProperty RootDirectory
    $commonFolders = @('Steam', 'Program Files\Steam', 'Program Files (x86)\Steam', 'SteamLibrary', 'Games\Steam')
    foreach ($drive in $drives) {
        foreach ($folder in $commonFolders) {
            $candidate = Join-Path $drive $folder | Join-Path -ChildPath $GameSub
            if (Test-Path $candidate) { return $candidate }
        }
    }
